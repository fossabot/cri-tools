...
const (
	apparmorProfilePrefix = "apparmor-test-"
	testProfiles          = `
...
var _ = framework.KubeDescribe("Security Context", func() {
	...
	if apparmor.IsAppArmorEnabled() {
		...
		Context("runtime should support apparmor [security context]", func() {
			...
			It("should fail with with an unloaded profile", func() {
				profile := apparmor.ProfileNamePrefix + "non-existant-profile"
				containerID := createContainerWithAppArmor(rc, ic, sandboxID, sandboxConfig, profile)
				checkContainerApparmor(rc, containerID, false)
			})

			It("should enforce a profile blocking writes", func() {
				profile := apparmor.ProfileNamePrefix + "cri-validate-apparmor-test-deny-write"
				containerID := createContainerWithAppArmor(rc, ic, sandboxID, sandboxConfig, profile)
				checkContainerApparmor(rc, containerID, false)
			})

			It("should enforce a permissive profile", func() {
				profile := apparmor.ProfileNamePrefix + "cri-validate-apparmor-test-audit-write"
				containerID := createContainerWithAppArmor(rc, ic, sandboxID, sandboxConfig, profile)
...
func createContainerWithAppArmor(rc internalapi.RuntimeService, ic internalapi.ImageManagerService, sandboxID string, sandboxConfig *runtimeapi.PodSandboxConfig, profile string) string {
	By("create a container with apparmor")
	containerName := apparmorProfilePrefix + framework.NewUUID()
...
func checkContainerApparmor(rc internalapi.RuntimeService, containerID string, shoudRun bool) {
	...
	if shoudRun {
		Expect(status.GetExitCode()).To(Equal(0))
	} else {
		Expect(status.GetExitCode()).NotTo(Equal(0))
	}
}
